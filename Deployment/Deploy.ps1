...
#copy the code commented below run it on azure cli
#
...
# -PublisherAdminUsers "relianceautomation@relianceinfosystems.com,aja@relianceinfosystems.com,mohammed@relianceinfosystems.com" `
#-Location "East US" 
...
if ($SQLDatabaseName -eq "") {
    $SQLDatabaseName = "RelianceSaaSDB"
}
...
Write-Host "Starting SaaS Deployment..."
...
if($LogoURLpng) { 
    ...
    Invoke-WebRequest -Uri $LogoURLpng -OutFile "../CustomerSite/wwwroot/contoso-sales.png"
    Invoke-WebRequest -Uri $LogoURLpng -OutFile "../AdminSite/wwwroot/contoso-sales.png"
...
if($LogoURLico) { 
    ...
    Invoke-WebRequest -Uri $LogoURLico -OutFile "../CustomerSite/wwwroot/favicon.ico"
    Invoke-WebRequest -Uri $LogoURLico -OutFile "../AdminSite/wwwroot/favicon.ico"
...
if (!(Test-Path '../Publish')) {		
	...
	dotnet publish ../AdminSite/AdminSite.csproj -c debug -o ../Publish/AdminSite/ -v q

	Write-host "   🔵 Preparing Metered Scheduler"
	dotnet publish ../MeteredTriggerJob/MeteredTriggerJob.csproj -c debug -o ../Publish/AdminSite/app_data/jobs/triggered/MeteredTriggerJob/ -v q --runtime win-x64 --self-contained true 

	Write-host "   🔵 Preparing Customer Site"
	dotnet publish ../CustomerSite/CustomerSite.csproj -c debug -o ../Publish/CustomerSite/ -v q
...
Set-Content -Path ../AdminSite/appsettings.Development.json -value "{`"ConnectionStrings`": {`"DefaultConnection`":`"$Connection`"}}"
dotnet-ef migrations script  --output script.sql --idempotent --context SaaSKitContext --project ../DataAccess/DataAccess.csproj --startup-project ../AdminSite/AdminSite.csproj
...
Write-host "   🔵 Clean up"
Remove-Item -Path ../AdminSite/appsettings.Development.json
